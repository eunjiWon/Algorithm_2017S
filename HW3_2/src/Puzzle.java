...
public class Puzzle implements Comparable<Puzzle> {
	...
	public Puzzle(int[][] given, int step, int h, Puzzle prev) {
		this.currentState = given;
		this.zero = findZero(given);
		this.walks = step;
		this.estimatedH = h;
		this.prev = prev;
	}
	...
	public static void aStar(int[][] givenState) {
		...
		pq.add(new Puzzle(givenState, 0, 0, null));
		Puzzle current = null;
		...
		while (!pq.isEmpty()) {
			...
			if (equlChk(current.currentState, goalState)) {
				...
				for (int[][] neighbour : availableMoves) {
					...
					if (!closedSet.contains(toString(neighbour))) {
						...
						pq.add(new Puzzle(neighbour, current.walks + 1, heuristicCost, current));
					} 
				}
			}
		}
	...
	private static boolean equlChk(int[][] a, int[][] b) {
		boolean equal = true;
	...
	public int compareTo(Puzzle other) {
		int p1 = this.walks + this.estimatedH;
		int p2 = other.walks + other.estimatedH;
		if (p1 < p2) {
			return -1;
		} else if (p1 > p2) {
			return 1;
