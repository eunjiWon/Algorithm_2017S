...
public class PuzzleM { 
	...
	private PuzzleM prev;

	public PuzzleM(int[][] given, int step, int h, PuzzleM prev) {
	...
	public static void aStar(int[][] givenState) {
		...
		PriorityQueue<PuzzleM> pq = new PriorityQueue<PuzzleM>(10, new Comparator<PuzzleM>(){
			@Override
			public int compare(PuzzleM a, PuzzleM b) {
		...
		pq.add(new PuzzleM(givenState, 0, 0, null));
		PuzzleM current = null;
			...
			if (isGoal(current.currentState) ){
				...
				for (int[][] neighbour : availableMoves) {
					...
					if (!closedSet.contains(toString(neighbour))) {
						...
						pq.add(new PuzzleM(neighbour, current.walks + 1, heuristicCost, current));
					} 
				}
			}
		}
