...
public class PuzzleN { 
	...
	private PuzzleN prev;

	public PuzzleN(int[][] given, int step, int h, PuzzleN prev) {
	...
	public static void aStar(int[][] givenState) {
		...
		PriorityQueue<PuzzleN> pq = new PriorityQueue<PuzzleN>(10, new Comparator<PuzzleN>(){
			@Override
			public int compare(PuzzleN a, PuzzleN b) {
		...
		pq.add(new PuzzleN(givenState, 0, 0, null));
		PuzzleN current = null;
...
//					if (!closedSet.contains(toString(neighbour))) {
						...
						pq.add(new PuzzleN(neighbour, current.walks + 1, heuristicCost, current));
				//} 
				}
			}
		}
